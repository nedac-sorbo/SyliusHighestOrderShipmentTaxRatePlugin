...
#                     PHP                        #
##################################################
FROM php:${PHP_VERSION}-fpm-alpine AS sylius_highest_order_shipment_tax_rate_plugin_php
...
RUN set -eux; \
    pip install yq; \
    yq -y -i '.imports[.imports|length] |= . + {"resource": "../vendor/nedac/sylius-highest-order-shipment-tax-rate-plugin/src/Resources/config/services_test.xml"}' config/services_test.yaml; \
    yq -y -i '.imports[.imports|length] |= . + "vendor/nedac/sylius-highest-order-shipment-tax-rate-plugin/tests/Behat/Resources/suites.yml"' behat.yml.dist; \
    yq -y -i '.default.extensions."Behat\\MinkExtension".base_url = "http://localhost/"' behat.yml.dist; \
    yq -y -i '.default.extensions."FriendsOfBehat\\SuiteSettingsExtension".paths = ["vendor/nedac/sylius-highest-order-shipment-tax-rate-plugin/features"]' behat.yml.dist
...
#                     NODEJS                     #
##################################################
FROM node:${NODE_VERSION}-alpine AS sylius_highest_order_shipment_tax_rate_plugin_nodejs
...
COPY --from=sylius_highest_order_shipment_tax_rate_plugin_php /srv/sylius/package.json ./package.json
COPY --from=sylius_highest_order_shipment_tax_rate_plugin_php /srv/sylius/gulpfile.babel.js ./gulpfile.babel.js
COPY --from=sylius_highest_order_shipment_tax_rate_plugin_php /srv/sylius/.babelrc ./.babelrc
COPY --from=sylius_highest_order_shipment_tax_rate_plugin_php /srv/sylius/.eslintrc.js ./.eslintrc.js
COPY --from=sylius_highest_order_shipment_tax_rate_plugin_php /srv/sylius/vendor/sylius/sylius/src/Sylius/Bundle/AdminBundle/gulpfile.babel.js ./vendor/sylius/sylius/src/Sylius/Bundle/AdminBundle/gulpfile.babel.js
COPY --from=sylius_highest_order_shipment_tax_rate_plugin_php /srv/sylius/vendor/sylius/sylius/src/Sylius/Bundle/AdminBundle/Resources/private vendor/sylius/sylius/src/Sylius/Bundle/AdminBundle/Resources/private/
...
#                     NGINX                      #
##################################################
FROM nginx:${NGINX_VERSION}-alpine AS sylius_highest_order_shipment_tax_rate_plugin_nginx
...
COPY --from=sylius_highest_order_shipment_tax_rate_plugin_php /srv/sylius/public public/
COPY --from=sylius_highest_order_shipment_tax_rate_plugin_nodejs /srv/sylius/public public/
...
#                     CHROME                     #
##################################################
FROM ubuntu:focal AS sylius_highest_order_shipment_tax_rate_plugin_chrome
